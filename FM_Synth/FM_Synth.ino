// FM synthesizer noodle - pin PA3 audio output //
...
const int OSCS = 6;
const int REVERB_BUFFER_SIZE = 0x1700;
const int TEMPO = (1 << 13);
...
signed char reverbBuffer[REVERB_BUFFER_SIZE];
...
void synth() {
  ...
  for (int i = 0; i < OSCS; i++)
  {
    ...
    else
    {
      ...
      params[i].outRev = (params[i].outWave * params[i].levelRev) >> FIXED_BITS;
    }
  }
}
...
void timerHandler(void) {
  ...
  for (int i = 0; i < OSCS; i++)
  {
    ...
    mixRev += params[i].outRev;
  }

  if ((reverbCounter & 1) == 0)
  {
    int reverb = (int)reverbBuffer[reverbAddr] << OUT_SHIFT_M1;
    reverb += mixRev;
    reverb >>= OUT_SHIFT;
  ...
  reverbCounter++;
  int out = (mix >> OUT_SHIFT) + reverbBuffer[reverbAddr] + OUT_OFFSET;
...
void setup() {
  ...
  params[0].level = CONST05 >> 1;
  params[1].level = CONST05;
