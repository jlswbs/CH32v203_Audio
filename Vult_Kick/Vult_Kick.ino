// Vult DSP Kick drum - pin PA3 audio output //
...
Kick_process_type kick;
  ...
  uint16_t gate, decay, pitch, swept, noise;

void timerHandler(void) {

	int sample = Kick_process(kick, gate, decay, pitch, swept, noise);
...
void loop() {
	...
	gate = 1;
  decay = random(1024, 16384);
  pitch = random(4096, 16384);
  swept = random(0, 32768);
  noise = random(0, 65535);
